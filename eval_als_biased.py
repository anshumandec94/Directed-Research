...
''' newrow=pd.DataFrame({"Algorithm" : ["ALS-20"], "Dataset" : [count], "RMSE": [eval(algo_20als,train,test)]})
    ...
    rmse_scores=rmse_scores.append(newrow)
'''
   # newrow=pd.DataFrame({"Algorithm" : ['ALS-15'], "Dataset":[count], "RMSE":[eval(algo_15als,train,test)]})
    #rmse_scores=rmse_scores.append(newrow)
   # newrow=pd.DataFrame({"Algorithm" : ['ALS-25'], "Dataset":[count], "RMSE":[eval(algo_25als,train,test)]})
    #rmse_scores=rmse_scores.append(newrow)
